...
/**
  ...
  */
class TestBeamSim @Inject()(private val actorSystem: ActorSystem,
                            private val beamServices: BeamServices
                       ) extends StartupListener with IterationStartsListener with IterationEndsListener with ShutdownListener {

  private val logger: Logger = LoggerFactory.getLogger(classOf[TestBeamSim])
  ...
  override def notifyStartup(event: StartupEvent): Unit = {
    ...
    val routerFuture = beamServices.registry ? Registry.Register("router", Props(classOf[TestBeamRouter], beamServices, fareCalculator))
